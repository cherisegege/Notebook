/**************************
 * USB网卡DM9621的测试例子
 * 和控制端点0建立通讯，利用厂商提供的命令获取或设置dm9621网卡寄存器的值；
 * 实现一个char驱动，通过ioctl命令来设置或读取寄存器的值
 * 驱动用usb_driver封装，调用usb接口库访问9621中的寄存器
 * Author: zht
 * Date: 2016-10-19
 **************************/
...
#include <linux/usb.h> //usb_driver & usb_device & usb_interface
...
//ioctl号
#define USB_TYPE	'U'
#define USB_R_REG	_IOWR(USB_TYPE, 1, struct usb_one_reg)
#define USB_W_REG	_IOW(USB_TYPE, 2, struct usb_one_reg)
#define USB_R_REGS	_IOWR(USB_TYPE, 3, struct usb_all_regs)
...
read_regs(char off, char length, void *data)
{
	...
	ret = usb_control_msg(udev,
		...
		DM_READ_REGS, //命令号，即00h
		0xC0, //命令类型，对应9621手册P40
...
write_one_reg(char reg, char value)
{
	return usb_control_msg(udev,
		...
		DM_WRITE_REG, //命令号，03H
		0x40, //命令类型，参9621手册P40
		value, reg, NULL, 0, 5000);
}


/**********************
 * file_operations->unlocked_iocl
 **********************/
static long
 ...
 **************************/
static void 
usb_reg_disconnect(struct usb_interface *intf)
{
...
//我支持谁
//USB_DEVICE宏用于匹配厂商ID和产品ID
static struct usb_device_id usb_ids[] = {
	{ USB_DEVICE(0x0A46, 0x9601), 0 }, //Davicom9601
	{ USB_DEVICE(0x0A46, 0x9621), 0 }, //Davicom9621
	{ }
};
