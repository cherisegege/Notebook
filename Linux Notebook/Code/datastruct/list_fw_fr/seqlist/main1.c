...
int main(void)
{
	...
	if(seplist_read(stulist, path)){
		printf("read information failed!\n");
	}
	...
	while (1) {
		...
		switch (choose) {
			case INSERT:
				...
				if(seqlist_insert_sort(stulist, &tmp, id_cmp)){
					printf("插入数据时产生错误\n");
					break;	
				}
//				seqlist_insert(stulist, &tmp);	
			...
			case SCAN:
				...
				break;
			case QUIT:			
				if(seqlist_save(stulist, path)){
					printf("save infotmaton failed!\n");
				}
